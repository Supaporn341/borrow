...
class OrderController extends Controller
{
    ...
    public function update(Request $request,$id){
        ...
        if ($status < $order->status){
            return response()->json(['message'=>'อย่ามั่ว อนุมัติไปแล้ว','status'=>422]);
        }

        if ($status == 1){
            if (empty($sign_length)) {
                return response()->json(['message'=>'กรุณาระบุลายเซ็น','status'=>200]);
            }
        ...
        return response()->json(['message'=>'บันทึกข้อมูลเรียบร้อย','status'=>200]);
